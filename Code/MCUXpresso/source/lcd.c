...
const char segments[10][8] = {"ABCDEF", "BC", "ABEDG", "ABCDG", "BCFG", "ACDDFG",
...
slcd_clock_config_t clkConfig = {kSLCD_AlternateClk1, kSLCD_AltClkDivFactor1,
kSLCD_ClkPrescaler01, false};
/* Hardware initialize. */
BOARD_InitPins();
BOARD_BootClockRUN();
...
pins[0] = (LCD_Pin) {0};
pins[1] = (LCD_Pin)
{ .mcu_pin=40, .waveForm=kSLCD_PhaseAActivate, .isBackPlane=1 }; /* SLCD COM1 ---
LCD_P40. */
pins[2] = (LCD_Pin)
{ .mcu_pin=52, .waveForm=kSLCD_PhaseBActivate, .isBackPlane=1 }; /* SLCD COM2 ---
LCD_P52. */
pins[3] = (LCD_Pin)
{ .mcu_pin=19, .waveForm=kSLCD_PhaseCActivate, .isBackPlane=1 }; /* SLCD COM3 ---
LCD_P19. */
pins[4] = (LCD_Pin)
{ .mcu_pin=18, .waveForm=kSLCD_PhaseDActivate, .isBackPlane=1 }; /* SLCD COM4 ---
LCD_P18. */
pins[5] = (LCD_Pin) { .mcu_pin=37, .waveForm=0, .isBackPlane=0 }; /* SLCD P05
--- LCD_P37. */
pins[6] = (LCD_Pin) { .mcu_pin=17, .waveForm=0, .isBackPlane=0 }; /* SLCD P06
--- LCD_P17. */
pins[7] = (LCD_Pin) { .mcu_pin=7, .waveForm=0, .isBackPlane=0 }; /* SLCD P07
--- LCD_P7. */
pins[8] = (LCD_Pin) { .mcu_pin=8, .waveForm=0, .isBackPlane=0 }; /* SLCD P08
--- LCD_P8. */
pins[9] = (LCD_Pin) { .mcu_pin=53, .waveForm=0, .isBackPlane=0 }; /* SLCD P09
--- LCD_P53. */
pins[10] = (LCD_Pin) { .mcu_pin=38, .waveForm=0, .isBackPlane=0 }; /* SLCD
P10 --- LCD_P38. */
pins[11] = (LCD_Pin) { .mcu_pin=10, .waveForm=0, .isBackPlane=0 }; /* SLCD
P11 --- LCD_P10. */
pins[12] = (LCD_Pin) { .mcu_pin=11, .waveForm=0, .isBackPlane=0 }; /* SLCD
P12 --- LCD_P11. */
/* Set SLCD back plane phase. */
SLCD_SetBackPlanePhase(LCD, 40, kSLCD_PhaseAActivate); /* SLCD COM1 ---
LCD_P40. */
SLCD_SetBackPlanePhase(LCD, 52, kSLCD_PhaseBActivate); /* SLCD COM2 ---
LCD_P52. */
SLCD_SetBackPlanePhase(LCD, 19, kSLCD_PhaseCActivate); /* SLCD COM3 ---
LCD_P19. */
SLCD_SetBackPlanePhase(LCD, 18, kSLCD_PhaseDActivate); /* SLCD COM4 ---
LCD_P18. */
...
void segmentToPinNumber(int dig_num, char seg_char, int* pin_num, uint8_t*
waveForm) {
*pin_num = 5 + 2*(dig_num-1); // See LCD-S401M16KR datasheet for segment topin conversions
...
SLCD_SetFrontPlaneSegments(LCD, pins[pin_num].mcu_pin,
pins[pin_num].waveForm);
}
// disp_num is the display digit number ranging from 1 to 4
// seg_char is letter corresponding to the desired segment ranging from A-F, P for decimal point and N for colon
